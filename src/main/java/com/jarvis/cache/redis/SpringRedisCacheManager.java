                ...
                for (MSetParam param : params) {
                    ...
                    if (null == hfield || hfield.isEmpty()) {
                        ...
                        if (expire == AbstractRedisCacheManager.NEVER_EXPIRE) {
                            ...
                            redisConnection.setEx(key, expire, val);
                        }
                    } else {
                        int hExpire = cacheManager.getHashExpire() < 0 ? result.getExpire() : cacheManager.getHashExpire();
                        redisConnection.hSet(key, AbstractRedisCacheManager.KEY_SERIALIZER.serialize(hfield), val);
                        if (hExpire > 0) {
                            redisConnection.expire(key, hExpire);
                        }
                    }
                }
            } finally {
            ...
            try {
                for (CacheKeyTO cacheKeyTO : keys) {
                    ...
                    if (null == hfield || hfield.isEmpty()) {
                        ...
                        redisConnection.hGet(key, AbstractRedisCacheManager.KEY_SERIALIZER.serialize(hfield));
                    }
                }
            } finally {
                if (redisConnection.isPipelined()) {
